...
class BracketNum:
    ...
    def __eq__(self, bn):
        if float(self.value) == float(bn.value) and self.bracket == bn.bracket:
            return True
        else:
            return False

    def complement(self, upperGuard):
        temp_value = self.value
        temp_bracket = None
        if float(self.value) == upperGuard:
            return BracketNum(str(upperGuard), Bracket.RO)  # ceil
        elif float(self.value) == 0 and self.bracket == Bracket.LC:
            return BracketNum('0', Bracket.LC)  # floor
        elif self.bracket == Bracket.LC:
            temp_bracket = Bracket.RO
        elif self.bracket == Bracket.RC:
            temp_bracket = Bracket.LO
        elif self.bracket == Bracket.LO:
            temp_bracket = Bracket.RC
        elif self.bracket == Bracket.RO:
